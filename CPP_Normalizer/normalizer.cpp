...
int setupPathAndFileName(){
	...
	if (strcmp (inputFileName, outputFileName) == 0){
		cout << endl << "Input and Output should have different names";
		cout << endl;
		return -1;
	}
	return 0;
...
int checkArguments(int argc){
	if (argc != NUM_ARGS) {
		cout << endl << "Give an argument" << endl;
...
int filecheck(SNDFILE* _inputFile){
	if(_inputFile == NULL){
		cout << endl << "Something went wrong with your audiofile";
		cout << endl;
		return -1;
	}
	return 0;
...
int setPeakLevel(char** argv){
	if (atof(argv[MAXPEAKLEVEL]) < 0 || atof(argv[MAXPEAKLEVEL]) > 1){
		cout << endl << "Output Level should be between 0 and 1";
        cout << endl;
		return -1;
	}
...
void giveInformation(){
	...
	cout << "Length in seconds: " << frames/samplerate << endl;
